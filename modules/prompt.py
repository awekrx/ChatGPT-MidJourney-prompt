...
class Prompt:
    ...
    def __create_prompt(self, text: str, words: int, model: int = 0):
        ...
        return open(f"./models/{model_name}.txt", "r").read().replace("NNN", str(words)).replace("TEXT", text)
    ...
    def ask(self,
        ...
        if not isinstance(content, int) and content < 0 and content > len(Settings.contents) - 1:
            raise ValueError(f"Content must be a int and between 0 and {len(Settings.contents) - 1}")
        
        if not isinstance(mj_model, int) and mj_model < 0 and mj_model > len(Settings.mj_models) - 1:
            raise ValueError(f"MidJourney model must be a int and between 0 and {len(Settings.mj_models) - 1}")
       
        if not isinstance(prompt_model, int) and prompt_model < 0 and prompt_model > len(Settings.prompt_models) - 1:
            raise ValueError(f"Prompt model must be a int and between 0 and {len(Settings.mj_models) - 1}")

        for color in colors:
            if not isinstance(color, int) and color < 0 and color > len(Settings.colors) - 1:
                raise ValueError(f"Color must be a int and between 0 and {len(Settings.colors) - 1}")
        ...
        try:
            for data in self.chatbot.ask(self.__create_prompt(text, count_words, prompt_model)):
    ...
    def __check_parameter(self, parameter: str, parameter_parent: list[dict], parameter_name: str):
        if parameter == "":
            parameter = 0
        elif parameter.isdigit():
            parameter = int(parameter)
            if parameter < 0 or parameter > len(parameter_parent) - 1:
                exit(f"[{RED}Error{RESET}] {parameter_name} must be between 1 and {len(parameter_parent) - 1}")
        else:
    ...
    def prompt(self):
        ...
        prompt_model = self.__check_parameter(prompt_model, Settings.prompt_models, "Prompt model")

        print(f"{GREEN}MidJourney model{RESET}")
        self.__show(Settings.mj_models)
        mj_model = input(f"-> ")
        mj_model = self.__check_parameter(mj_model, Settings.mj_models, "MidJourney model")
        

        print(f"{GREEN}Image description{RESET}")
        prompt: str = input(f"-> {BOLD}")

        print(f"{RESET}{GREEN}Description count (The more, the more details and more chaos){RESET}")
        count: str = input(f"-> ")
        if count.isdigit():
            count = int(count)
        else:
        ...
        if not mj_model == 0:
            prompt = self.ask(text=prompt, count_words=count, prompt_model=prompt_model, mj_model=mj_model)
        else:
            print(f"{RESET}{GREEN}Advanced settings{RESET}")
            self.__show(Settings.advanced_settings)
            advanced: str = input(f"-> ")
            if advanced == "1":
                prompt = self.ask(text=prompt, count_words=count, prompt_model=prompt_model)
                ...
                renderer = input(f"-> ")
                renderer = self.__check_parameter(renderer, Settings.renderers, "Renderer")
                ...
                prompt = self.ask(text=prompt, count_words=count, type=type, resolution=resolution, renderer=renderer, content=content, prompt_model=prompt_model, colors=color_nubmers)
